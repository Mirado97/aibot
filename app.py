...
TRAIL_TRIGGER = 0.010   # +1% в пользу → SL в безубыток
TRAIL_DIST    = 0.010   # трейлинг на 1% от пика
TRAIL_TP      = 0.050   # потолок TP для трейлинг-версии (5%)
...
# Фикс: SL 1%, TP 2%
_trades_fix, _equity_fix = mb.run_backtest(_df, sl_pct=SL_PCT, tp_pct=TP_PCT)
_stats_fix = mb.calc_stats(_trades_fix, _equity_fix)

# Трейлинг: SL 1%, трейлинг с 1% от пика, потолок TP 5%
...
print(f"Фикс: {_stats_fix['n_trades']} сделок  PF={_stats_fix['profit_factor']:.2f}", flush=True)
print(f"Трейл: {_stats_tr['n_trades']} сделок  PF={_stats_tr['profit_factor']:.2f}", flush=True)


def _rows_html(trades, df) -> str:
    rows = ""
    for t in list(t for t in trades if t.exit_reason == "end_of_data") + \
             list(reversed([t for t in trades if t.exit_reason != "end_of_data"])):
        sc  = "#2196F3" if t.side == "long" else "#FF9800"
        pc  = "#4CAF50" if t.is_win else "#ef5350"
        xts = df.index[t.exit_bar].strftime("%m-%d %H:%M") if t.exit_bar else "—"
        xpx = f"{t.exit_price:.2f}" if t.exit_price else "—"
        rows += (
            f"<tr>"
            f"<td>{df.index[t.entry_bar].strftime('%m-%d %H:%M')}</td>"
            f"<td>{xts}</td>"
...
def _make_page() -> str:
    rows_fix = _rows_html(_trades_fix, _df)
    rows_tr  = _rows_html(_trades_tr,  _df)
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  h2{{padding:8px 12px 4px;font-size:13px;color:#9ca3af}}
...
{_stats_row(_stats_fix, f"Фикс TP {TP_PCT*100:.1f}%")}
...
<div class="wrap">
  <p style="padding:8px 0 4px;font-size:13px">Ордера — Трейлинг ({len(_trades_tr)})</p>
  <table>
    <tr><th>Вход</th><th>Выход</th><th>Сторона</th><th>Вход $</th><th>Выход $</th><th>PnL %</th><th>Причина</th></tr>
    {rows_tr}
  </table>
